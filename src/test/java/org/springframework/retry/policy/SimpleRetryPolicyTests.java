...
public class SimpleRetryPolicyTests {
	...
	@Test
	public void testExceptedExceptionCanRetry() throws Exception {
		final SimpleRetryPolicy simpleRetryPolicy = new SimpleRetryPolicy(3, Collections
				.<Class<? extends Throwable>, Boolean>singletonMap(RuntimeException.class, true));
		final RetryContext context = simpleRetryPolicy.open(null);
		assertTrue(simpleRetryPolicy.canRetry(context));

		simpleRetryPolicy.registerThrowable(context, new IllegalArgumentException());
		assertTrue(simpleRetryPolicy.canRetry(context));

		simpleRetryPolicy.setMaxAttempts(0);
		assertFalse(simpleRetryPolicy.canRetry(context));
